...
def make_sequences_for_eruption(
    ...
    """
    Construit les séquences X et la cible y.

    Pour chaque séquence :
        X_i = features de [t-seq_len+1, ..., t]
        y_i = heures avant éruption à t

    Aucun point futur n'est utilisé dans X.

    sequence_stride permet de réduire la redondance entre séquences.
    Exemple :
        sequence_stride=1  -> une séquence par minute
        sequence_stride=5  -> une séquence toutes les 5 minutes
        sequence_stride=10 -> une séquence toutes les 10 minutes
    """
    ...
    for end_idx in range(seq_len - 1, len(feature_table), sequence_stride):
        ...
        # On garde les NaN ici.
        # L'imputation sera fit uniquement sur train après split.
        X_list.append(seq)
...
def main(args):
    ...
    required_meta = {"eruption_id", "eruption_start_utc"}
    ...
    for _, row in meta.iterrows():
        ...
        if len(y) == 0:
            print("      WARNING : aucune séquence produite pour cette éruption.")
            continue
    ...
    config = {
        ...
        "target": "continuous delay in hours before eruption",
        "leakage_controls": [
